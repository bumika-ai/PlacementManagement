...
if(isset($_POST['submit']))
{
	...
	if($Id_stmt->num_rows()>0)
	{
		?><script type="text/javascript">
			alert("webinar already exists")
		</script>
		<?php
	}
	else
	{
		$sql="INSERT INTO workshop(WORKSHOP_ID,WORKSHOP_NAME,COMPANY_ID,DESCRIPTION,SKILLS,START_DATE,TIMING) VALUES (?,?,?,?,?,?,?)";
		$stmt=$conn->prepare($sql);
		$stmt->bind_param('sssssss',$Workshop_id,$Workshop_name,$Company_id,$Description,$Skill,$Start_date,$Timing);
		$result=$stmt->execute();
		if($result>0)
		{?><script type="text/javascript">
			alert("Insertion sucessfully");</script><?php
		}
	}
}
?><!DOCTYPE html>
...
<body>
	<form action="webinar.php" method="post">
		<fieldset>
